...
**Accurate.** Next.js server components proxy to the old media server/the old request app/qBittorrent. No CORS issues, API keys server-side. Correct.
...
### the TV automation suite / the movie automation suite / the indexer bridge / the subtitle automation suite — Internal URL and network_mode
...
### the old request app proxy route

**Partially outdated — webhook resolved, catch-all note stands.** CLAUDE.md page map (Section 5) shows `api/the old request app/[...path]/route.ts` as a the old request app proxy route. This catch-all file does not exist. The app has fully replaced the old request app's request functionality with the native request system in `src/lib/requests/` and `src/app/api/requests/`. No `/api/the old request app/...` catch-all proxy exists.

> **RESOLVED (webhook) v0.9.1, 2026-06-04:** The the old request app webhook route listed in Section 14 (`/api/the old request app/webhook`) **does** now exist at `src/app/api/the old request app/webhook/route.ts`. It implements HMAC-SHA256 verification and handles `MEDIA_APPROVED`, `REQUEST_APPROVED`, `MEDIA_AVAILABLE` events. See "Resolved Since Audit" section below.

### the old media server proxy route

**Wrong.** CLAUDE.md page map shows `api/the old media server/[...path]/route.ts` as a single catch-all proxy. The actual implementation uses specific sub-routes:
- `api/the old media server/continue-watching/`
- `api/the old media server/image/[itemId]/`
- `api/the old media server/playback/[id]/`
- `api/the old media server/seasons/[seasonId]/episodes/`
- `api/the old media server/series/[id]/next-episode/`
- `api/the old media server/series/[id]/seasons/`
- `api/the old media server/sessions/playing/`, `.../progress/`, `.../stopped/`
- `api/the old media server/stream/[...path]/`
- `api/the old media server/subtitles/[itemId]/[streamIndex]/`

There is no `api/the old media server/[...path]` catch-all.

### the old media server image proxy path

**Wrong.** Section 6 Phase 2 says to create `app/api/the old media server-image/[itemId]/route.ts`. The actual route lives at `src/app/api/the old media server/image/[itemId]/route.ts` — it's a sub-route under the `the old media server/` namespace, not a top-level `the old media server-image/` route.
...
| `zustand` | `5+` | `^5.0.14` | `5+` ✓ |
| `the legacy media-server SDK` | `latest unstable or stable` | `^0.13.0` | unchanged |
...
|---|---|
| `api/the old media server/[...path]/route.ts` — catch-all proxy | Does not exist; replaced by specific sub-routes (see Section 3 above) |
| `api/the old request app/[...path]/route.ts` — the old request app proxy | Does not exist at all |
...
**Wrong path.** "configure a server-scoped the old media server API client in `lib/the old media server.ts`" — actual file is `src/lib/the old media server/client.ts`. There is no top-level `lib/the old media server.ts`.

### Phase 3 — the old request app integration

**Outdated.** "Install no new packages; use fetch wrapper in `lib/the old request app.ts`" — `lib/the old request app.ts` does not exist. The app does not proxy the old request app for user-facing requests; it uses the native request system (`src/lib/requests/`) instead.
...
### external automation stack network_mode: host

> **RESOLVED v0.9.1, 2026-06-04:** CLAUDE.md was corrected. The external automation stack section now reads "All external automation stack (the TV automation suite, the movie automation suite, the indexer bridge, the subtitle automation suite) run on the `compose_default` bridge network… reachable by both container name and host IP." `docker inspect the TV automation suite` confirms `NetworkMode: compose_default`. The prose and table in CLAUDE.md now agree. See "Resolved Since Audit" section below.

~~**Wrong.** As documented under Section 3: the TV automation suite, the movie automation suite, the indexer bridge, and the subtitle automation suite use bridge networking with port bindings in the actual compose file, not `network_mode: host`. They are reachable by container name within the Docker bridge network.~~
...
**Accurate for all other vars.** `a retired env var`, `a retired env var`, `LEGACY_MEDIA_URL`, `LEGACY_MEDIA_API_KEY`, `LEGACY_MEDIA_USER_ID`, `LEGACY_TV_*`, `LEGACY_MOVIE_*`, `LEGACY_INDEXER_*`, `LEGACY_SUBS_*`, `NEXT_PUBLIC_APP_URL`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`, `DB_PATH`, SMTP vars — all match source code usage.
...
**Accurate.** `src/lib/the old media server/playback.ts` exists. The description of `getPlaybackData()`, quality tiers, direct play logic, and `VideoPlayer` quality switching matches the actual component at `src/components/media/VideoPlayer.tsx`.
...
**Mostly accurate** as a backlog. One note: "the external automation services status — uses `http://the TV automation suite:8989` and `http://the movie automation suite:7878` (both on the Docker bridge network)" — this is correct about network reachability (they are on bridge), but the Section 3 text says to use the host IP. This is internally inconsistent (the backlog note is the accurate one).
...
1. **the old media server no catch-all proxy** — CLAUDE.md page map shows `api/the old media server/[...path]/route.ts`. This does not exist. Seven specific sub-routes replace it: `continue-watching/`, `image/[itemId]/`, `playback/[id]/`, `seasons/[seasonId]/episodes/`, `series/[id]/next-episode/`, `series/[id]/seasons/`, `sessions/playing/`+`progress/`+`stopped/`, `stream/[...path]/`, `subtitles/[itemId]/[streamIndex]/`. Medium impact for developers adding new the old media server features.

2. **Phase 2/3/4 lib file paths wrong** — Build phases section references `lib/the old media server.ts`, `lib/the old request app.ts`, `lib/qbt.ts`. These do not exist. Actual paths: `lib/the old media server/client.ts`, no the old request app client (native requests only), `lib/qbittorrent/session.ts`. Low-to-medium impact: dev environment only.
...
**3. No the old request app webhook route existed (was HIGH)**
- Original finding: `/api/the old request app/webhook` endpoint listed in Section 14 did not exist.
- Resolution: `src/app/api/the old request app/webhook/route.ts` implemented in v0.9.1. Handles `MEDIA_APPROVED`, `REQUEST_APPROVED`, `MEDIA_AVAILABLE`. HMAC-SHA256 verification via `timingSafeEqual` when `the retired webhook secret` is set.
...
**5. `external automation` services documented as `network_mode: host` (was MEDIUM)**
- Original finding: CLAUDE.md prose said all four external automation stack use host networking; the internal address table contradicted this.
- Resolution: CLAUDE.md updated. external automation stack section now says bridge network, reachable by container name or host IP. Table and prose agree.
- Confirmed: `docker inspect the TV automation suite` shows `NetworkMode: compose_default`.
