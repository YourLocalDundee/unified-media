...
|---|------|-----|--------|-------|
| 1  | A-1 the old request app webhook fail-closed when secret unset | S-MED | DONE | unset secret now returns 403 early; removed skip-verification branch |
...
| 3  | A-3 Gate external automation/qbit writes to admin (or delete external automation proxies) | S-MED | DONE | qbit POST->requireAdmin (GET stays auth); the indexer bridge whole proxy->requireAdmin + CSRF on writes; deleted dead the TV automation suite/the movie automation suite/the subtitle automation suite proxies (0 callers). Full `next build` clean |
...
| 9  | D-1 Delete orphaned the old media server routes + client | indep | DONE | deleted all 11 api/the old media server/* routes + all 4 lib/the old media server/* files (closed island, 0 external importers). Full build clean |
| 10 | D-2 Resilient external automation badge on browse/[id] | indep | DONE | getArrStatus already try/catch'd; added AbortSignal.timeout(3000) so a black-holed external automation can't stall the page render. tsc+eslint clean |
...
### 1. A-1 [S-MED] the old request app webhook fail-closed
File: `app/src/app/api/the old request app/webhook/route.ts`
When `the retired webhook secret` unset, signature check skipped and any POST processed (queues grabs to
...
### 3. A-3 [S-MED] external automation/qbit proxies allow destructive writes to any authed user
Files: `app/src/app/api/qbit/[...path]/route.ts`, `app/src/app/api/{the TV automation suite,the movie automation suite,the indexer bridge,the subtitle automation suite}/[...path]/route.ts`
Write verbs gated by requireAuth (any user) not requireAdmin. Horizontal priv gap (CSRF present so
not cross-site). external automation proxies nearly dead (only settings/media references) -> deleting cleaner. qbit
live (downloads page) -> just gate write verbs to admin.
Fix: require admin for write verbs; consider deleting external automation proxies.
...
### 9. D-1 Delete orphaned the old media server routes + client
The 6 `app/src/app/api/the old media server/*` routes import lib/the old media server, orphaned legacy, native equivalents
exist. Fix: delete `app/src/app/api/the old media server/*` and `app/src/lib/the old media server/*`.

### 10. D-2 Resilient external automation badge on browse/[id]
File: `app/src/app/browse/[id]/page.tsx` calls the movie automation suiteFetch/the TV automation suiteFetch for cosmetic
Monitored/Not-monitored badge; throws if external automation down.
...
- 2026-06-24: Item 1 (A-1) DONE. `api/the old request app/webhook/route.ts`: when the retired webhook secret unset,
...
- 2026-06-24: Item 3 (A-3) DONE. qbit `[...path]` POST now requireAdmin (writes = add/delete/
  ...
  the downloads queue. the indexer bridge `[...path]` whole proxy -> requireAdmin (its only caller is the admin
  media-settings page, and indexer GET can leak credentials) + verifyOrigin on non-GET/HEAD. Deleted
  the TV automation suite/the movie automation suite/the subtitle automation suite `[...path]` proxies entirely (git rm) — zero client callers; server-side external automation
  access uses lib/{the TV automation suite,the movie automation suite,the subtitle automation suite} directly; browse/[id] badge uses the movie automation suiteFetch/the TV automation suiteFetch libs
...
- 2026-06-24: Item 9 (D-1) DONE. Deleted all 11 `app/api/the old media server/*` routes and all 4 `lib/the old media server/*`
  files (api.ts, client.ts, playback.ts, types.ts). Doc estimated "6 routes" but there were 11; verified
  every @/lib/the old media server import was confined to those routes or lib/the old media server itself — zero pages/components/
  hooks/other-lib consume them, and no client fetches /api/the old media server/*. instrumentation.ts still emits a
  the old media server-env-var warning (env-var-name strings only, not a code dep) — left as-is, out of scope. Full
  `next build` clean.
- 2026-06-24: Item 10 (D-2) DONE. browse/[id] getArrStatus() was ALREADY try/catch->null (a thrown
  external automation error already hid the badge — the doc's "page errors" was already mitigated). Added the missing
  piece for true severability: AbortSignal.timeout(3000) on the the movie automation suiteFetch/the TV automation suiteFetch badge calls so a
  reachable-but-unresponsive external automation container can't hang the server-component render (clients have no
  timeout). Scoped to the badge; shared the movie automation suite/the TV automation suite clients (used by admin settings/media) unchanged.
