...
| ----- | ---- | ---------- |
| 1 — Scaffolding | App runs in Docker, reachable at `<media-host>`, auth available | Loads + redirects to `/login` if unauthenticated |
| 2 — the old media server | Browse library, detail pages, play content | Browse Movies, open detail, play a file |
| 3 — the old request app | Search non-library content + submit requests | Search → not in library → request → appears in `/requests` |
...
## Independence Build (native replacement of external external automation + the old media server)
...
| ----- | -------- | -------- | ----------- |
| 1 — Indexer Aggregation | the indexer bridge | `src/lib/indexer/` | `/admin/indexers` |
| 2 — Download Automation | the TV automation suite + the movie automation suite | `src/lib/automation/` | `/admin/automation` |
| 3 — Request Bridge | the old request app→external automation link | `src/lib/automation/bridge.ts` | `/admin/automation/bridge` |
| 4 — Subtitle Management | the subtitle automation suite | `src/lib/subtitle/` | `/admin/subtitles` |
| 5 — Media Server | the old media server | `src/lib/media-server/` | `/admin/media-server` |
| 6 — Browse/Watch wired to native media server | the old media server browse/watch UX | — | — |
| 7 — Native Request Management | the old request app requests | `src/lib/requests/` | `/admin/requests` |
...
| -------- | ----- | -------- | ------- |
| `LEGACY_MEDIA_USER_ID` | 3 | Yes | Admin user UUID — `GET /Users/Me` → `Id` |
| `the retired webhook secret` | 3 | Optional | Verifies `X-Webhook-Signature` on webhook POSTs |
...
| the external automation services fully removed + the indexer bridge-direct admin surface removed | v0.11.10 | CLAUDE.md §3 |
...
- **the external automation services status** — read-only monitoring status on media detail pages (shipped as part of the
  independence build).
