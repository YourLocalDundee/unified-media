...
the full grab-confirmation flow (real the indexer bridge/indexer search, real qBittorrent grab attempt),
...
| /admin/indexers | ✅ | wa-50-admin-indexers.png | 10 real indexers with live health status (EZTV/Nyaa OK, one the indexer bridge proxy in Error state — real backend condition, not a driver artifact), enable toggles, Test buttons |
