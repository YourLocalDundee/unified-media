...
- **S1/S2** — CSRF on every mutating route (qbit/torznab/the old media server metadata + all admin/automation/
...
Note: `stream`/`playback`/`subtitles`/`sessions/*` the old media server routes were already gated via
...
- **P0** — auth on qbit/the old media server/torznab routes (+ `proxy.ts` validates sessions); enforce
