...
| `the legacy media-server SDK` | `^0.13.0` | Current upstream is 0.13.x. CLAUDE.md references "unstable" builds — this is the stable release channel. OK. |
...
| `a retired env var` / `a retired env var` | Not in .env.local, not read in src/ | CLAUDE.md documents these and the Phase 1 example compose snippet references them, but no `process.env.LEGACY_REQ_*` calls exist anywhere in `src/`. The the old request app integration appears to have been absorbed into the independence build (Phase 7 native requests). If external the old request app integration is still active via hardcoded URLs in proxy routes, this is a documentation drift issue. |
...
**`a retired env var` / `a retired env var` absence:** The compose service does not pass `a retired env var` or `a retired env var`, and as noted in section 3, no code in `src/` reads those vars. This is consistent.
