...
### A2-009 — `browse/[id]` blocks render on the external automation services lookup with no timeout
- **Severity:** MEDIUM
- **File:** `src/app/browse/[id]/page.tsx:23-40,84` (`await getArrStatus(item)` in the server render path)
- **What's wrong:** The browse acquisition detail page `await`s `getArrStatus`, which calls `the movie automation suiteFetch`/`the TV automation suiteFetch` (network to `external automation` services) inside the page render before returning any HTML. The call is `try/catch`-guarded so a failure degrades to no badge, but there is no timeout — if the external automation services is slow or hung, the whole detail page TTFB stalls. It is also outside any `Suspense` boundary (the page returns one tree), so nothing renders until the `external automation` round-trip resolves or the socket times out at the default fetch timeout.
- **Why it matters:** The monitoring badge is a non-essential adornment, yet it gates the entire detail page's first byte on a third-party service. A flaky external automation container makes browse detail pages feel broken.
- **Suggested fix:** Wrap the `external automation` badge in its own `<Suspense>` async sub-component so the rest of the page streams immediately, and/or add an `AbortSignal.timeout(2000)` to `getArrStatus`'s fetches so it degrades fast.
