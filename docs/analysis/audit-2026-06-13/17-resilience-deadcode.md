...
9/11 Playback prefs, Torrent→Interface tab, Advanced the old media server override, store `browsePageSize`,
...
| B20 | Advanced → the old media server URL override | no-op | `unified-the old media server-url-override` referenced only by its own write + `clearAllPreferences`; no stream builder reads it | 08 (A8-M2) | wire into client stream src, or remove |
