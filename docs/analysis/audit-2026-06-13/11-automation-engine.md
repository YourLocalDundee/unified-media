# Audit 11 — Automation Engine (the external automation services-replacement brain)

Audit date: 2026-06-13. Scope: `src/lib/automation/*` + `src/app/api/automation/*`, traced end-to-end
through the request-approval grab triggers (`approve`, `auto-approve`, `the old request app/webhook`, `requests` POST).
...
user owns, a separate long-term request, an external automation-side import outside this app), auto-delete will `fs.unlinkSync`
...
`the old request app/webhook/route.ts:146-150`
...
**Why it matters.** The product is sold (CLAUDE.md §14) as a the external automation services replacement; cutoff-based upgrades
