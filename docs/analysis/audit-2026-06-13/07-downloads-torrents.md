...
`requireAuth()` check** — unlike every sibling proxy (the TV automation suite/the movie automation suite/the indexer bridge/the subtitle automation suite). Any
...
- **What's wrong:** Neither the `GET` nor `POST` handler calls `requireAuth()`. Compare the sibling
  proxies, all of which gate at the handler: `src/app/api/the TV automation suite/[...path]/route.ts:14`
  (`await requireAuth()`), and likewise the movie automation suite/the indexer bridge/the subtitle automation suite. The `/downloads` *page* is gated by
...
- **Suggested fix:** Add `await requireAuth()` at the top of both `GET` and `POST` in the qbit proxy
  ...
  as the the TV automation suite proxy does. Consider `requireAdmin()` for the mutating preference endpoints.
