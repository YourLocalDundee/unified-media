...
### A4-C2 — Unauthenticated the old media server proxy/stream/session routes (credentialed SSRF surface)
**Severity:** CRITICAL
**Files:**
- `src/app/api/the old media server/playback/[id]/route.ts` — no `requireAuth`/`getSession` (verified: grep finds none)
- `src/app/api/the old media server/sessions/playing/route.ts`, `.../progress/route.ts`, `.../stopped/route.ts` — no auth; blindly forward the JSON body to the old media server
- `src/app/api/the old media server/subtitles/[itemId]/[streamIndex]/route.ts:10-31` — no auth; builds `${LEGACY_MEDIA_URL}/Videos/${itemId}/Subtitles/${streamIndex}/Stream.vtt` from unsanitised params

**What's wrong:** These handlers call `the old media serverFetch(...)` / `fetch(...)` with the server-side `LEGACY_MEDIA_API_KEY` but never establish a session. Anyone who can reach the app (it is internet-exposed via Caddy/BunkerWeb with auth handled *inside* the app) can hit `/api/the old media server/playback/<anything>`, `/api/the old media server/sessions/*`, and `/api/the old media server/subtitles/<itemId>/<idx>` without logging in. The session routes forward an attacker-controlled body to the old media server's `/Sessions/*` endpoints under the admin key; the subtitle route interpolates `itemId`/`streamIndex` straight into the the old media server URL. Contrast with `api/the old media server/stream/[...path]` which *does* gate on `getSession()` (line 20-23) and `continue-watching` which uses `requireAuth()` — so the inconsistency is clearly an oversight.
...
**Suggested fix:** Add `await requireAuth()` (or `const s = await getSession(); if (!s) return 401`) as the first line of each handler, matching `stream/[...path]/route.ts`. Validate `streamIndex` is a non-negative integer and `itemId` matches the the old media server GUID shape before interpolation.
...
- Stream proxy (`the old media server/stream/[...path]`) strips client `api_key` and injects server key (line 33-38), and rewrites manifest segment URLs through the proxy.
