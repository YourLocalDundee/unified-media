...
**(HIGH) The the indexer bridge proxy is auth-gated only by `requireAuth` (any logged-in user), not admin**, and
...
the indexer bridge discovery puts the API key in the stored URL and the DB).
...
`page.tsx:520`, not a password field). For the indexer bridge-discovered indexers the `api_key` column holds
the **the indexer bridge master API key** (see A12-08), so that master key is shipped to the browser too.
...
passkey leak can get the underlying tracker account banned. The the indexer bridge master key grants full
control of the the indexer bridge instance.
...
### A12-04 — the indexer bridge proxy authorizes any logged-in user (not admin) and double-encodes JSON
**Severity:** HIGH
**File:** `src/app/api/the indexer bridge/[...path]/route.ts:13-46`

**What's wrong:** Two issues. (1) The proxy gates on `await requireAuth()` (`:14`) — **any
authenticated non-admin user** can call `POST/PUT/DELETE /api/the indexer bridge/<anything>` and the server
injects the the indexer bridge master `X-Api-Key` (`:19`). That hands every regular user full write access to
the indexer bridge (delete indexers, change configs, trigger searches) under the server's privileged key. (2)
The success path does `const data = ct.includes('application/json') ? await res.json() : await
res.text()` then `NextResponse.json(data, ...)` (`:35-36`). When the indexer bridge already returns JSON this
...
the original text, and any non-JSON/HTML error page from the indexer bridge is corrupted. There is also no path
...
session and the the indexer bridge admin API, and it lets everyone through. The double-encoding masks real
...
### A12-08 — the indexer bridge discovery embeds the master API key into the stored URL and the `api_key` column
**Severity:** MEDIUM (secret handling)
**File:** `src/lib/indexer/discovery.ts:86-97`.

**What's wrong:** For each discovered the indexer bridge indexer it stores
`torznab_url = `${the indexer bridgeUrl}/${idx.id}/api?apikey=${the indexer bridgeKey}`` and `api_key = the indexer bridgeKey`. The
master the indexer bridge key is now persisted twice (in the URL query string and the key column) for every
discovered row. Combined with A12-01, that key is then shipped to the browser on GET. Separately the
constructed Torznab URL is almost certainly wrong: the indexer bridge's per-indexer Torznab path is
`/{id}/api` under `/api/v1/indexer` only via the newznab download endpoint — `${the indexer bridgeUrl}/${id}/api`
omits the `/api/v1/indexer` (or `/download`) segment the indexer bridge expects, so these rows likely 404 on
search.
...
at query time. Verify the actual the indexer bridge Torznab endpoint shape against a live instance before
...
Torznab/Newznab feeds (the Torznab proxy, some the indexer bridge definitions) emit mixed case (`magnetUrl`, `infoHash`)
...
**Why it matters:** Results from common Torznab indexers may have no usable download target
...
- **Indexer CRUD authz:** `/api/indexer`, `/[id]`, `/[id]/test`, `/[id]/activate` all call
  `requireAdmin()`. ✔ (the gap is the the indexer bridge proxy A12-04 and torznab/search A12-02).
