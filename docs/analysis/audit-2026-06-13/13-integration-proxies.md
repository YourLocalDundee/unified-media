...
`src/lib/{the movie automation suite,the TV automation suite,the subtitle automation suite,the old media server}` client/api modules. the old media server stream/playback/subtitles/
sessions and SMTP/notifications were out of scope and skipped.

## Summary

The good news first: the three catch-all `[...path]` proxies (the movie automation suite, the TV automation suite, the subtitle automation suite) **are**
...
echoed in success bodies. The the old request app webhook does proper HMAC-SHA256 verification with a constant-time
compare when a secret is set.

The bad news: **four the old media server metadata routes ship with no `requireAuth()` at all** while their own
sibling route (`continue-watching`) has it — an open, key-injecting relay into the old media server reachable by
...
into a the old media server query string. Secondary issues: the `[...path]` proxies authorize any logged-in user
(open registration is on by default) for full read/write control of the external automation services/the subtitle automation suite including
...
### A13-01 — the old media server metadata routes are unauthenticated key-injecting proxies (open relay)
- **Severity:** CRITICAL
- **File:**
  - `src/app/api/the old media server/image/[itemId]/route.ts:9` (no auth; whole handler)
  - `src/app/api/the old media server/series/[id]/next-episode/route.ts:17`
  - `src/app/api/the old media server/series/[id]/seasons/route.ts:8`
  - `src/app/api/the old media server/seasons/[seasonId]/episodes/route.ts:9`
- **What's wrong:** None of these four route handlers call `requireAuth()` (or any auth). They
  inject the server-held `LEGACY_MEDIA_API_KEY` (`image` route lines 36–40; the other three via
  `the old media serverFetch`/`getSeasons`/`getEpisodes` which embed the MediaBrowser `Token=` header in
  `client.ts:13-15`) and return the old media server data to whoever calls them. The app's only gate in front of
...
### A13-02 — external automation proxies authorize any logged-in user for full read/write incl. delete
- **Severity:** HIGH
- **File:** `src/app/api/the movie automation suite/[...path]/route.ts:13`, `the TV automation suite/[...path]/route.ts:13`,
  `the subtitle automation suite/[...path]/route.ts:14` (each calls `requireAuth()`, not `requireAdmin()`)
- **What's wrong:** The catch-all proxies gate on `requireAuth()` (any active session) and then
  ...
  `proxy()` (`the movie automation suite/route.ts:41-44`, etc.). There is no per-method or admin restriction. Open
  enrollment is the default (`CLAUDE.md` §2: "Open enrollment, email verification optional … default
  false"), so any self-registered, non-admin user gets the server's the external automation services/the subtitle automation suite admin API
  ...
  profiles, etc. The lib layer (`lib/the movie automation suite/api.ts:33` `deleteMovie`, `:47` `commandSearch`;
  `lib/the TV automation suite/api.ts:34,53`) confirms these destructive operations are first-class.
- **Why it matters:** Privilege escalation from "any user" to "full external automation administrator." A normal
  ...
  privilege level.
- **Suggested fix:** Gate the external automation proxies on `requireAdmin()`, or split read (`GET`, any user) from
...
- **Severity:** HIGH
- **File:** `the movie automation suite/[...path]/route.ts:27`, `the TV automation suite/[...path]/route.ts:29`,
  `the subtitle automation suite/[...path]/route.ts:28`, `the old media server/image/[itemId]/route.ts:43`,
  `lib/the old media server/client.ts:54`, `lib/the movie automation suite/client.ts:22`, `lib/the TV automation suite/client.ts:22`,
  `lib/the subtitle automation suite/client.ts:22`
- **What's wrong:** Every `fetch()` to an upstream service is issued with no `AbortSignal` /
  `signal: AbortSignal.timeout(...)` and no connection deadline. If the external automation services/the subtitle automation suite/the old media server is
...
- **Severity:** MEDIUM
- **File:** `the movie automation suite/[...path]/route.ts:34-35`, `the TV automation suite/[...path]/route.ts:36-37`,
  `the subtitle automation suite/[...path]/route.ts:35-36`; lib mirror: `lib/the movie automation suite/client.ts:28-31`,
  `lib/the TV automation suite/client.ts:28-31`, `lib/the subtitle automation suite/client.ts:28-31`, `lib/the old media server/client.ts:56-60`
- **What's wrong:** On a non-2xx upstream response the proxy reads the upstream body and returns it
  ...
  NextResponse.json(data, { status: res.status })`). external automation/the old media server error payloads frequently embed
...
### A13-05 — `next-episode` interpolates the `id` path param into a the old media server query string (query injection) + unauthenticated
- **Severity:** MEDIUM
- **File:** `src/app/api/the old media server/series/[id]/next-episode/route.ts:24-26`
- **What's wrong:** `id` (a raw path segment) is concatenated directly into the the old media server query
  ...
  alters the old media server's query semantics. Combined with A13-01 (this route has no auth), the injection is
  reachable pre-authentication. (Verified: `new URL()` preserves the injected `&`/`#` in the query
  component; the host stays fixed.)
- **Why it matters:** An attacker can manipulate the upstream the old media server request beyond the intended
...
- **Severity:** MEDIUM
- **File:** `the movie automation suite/[...path]/route.ts:15,27`, `the TV automation suite/[...path]/route.ts:15,29`,
...
### A13-07 — the old request app webhook can be triggered by anyone when `the retired webhook secret` is unset
- **Severity:** MEDIUM
- **File:** `src/app/api/the old request app/webhook/route.ts:82-91`
- **What's wrong:** Signature verification is conditional: `if (secret) { …verify… } else { …WARNING
  log, skip… }`. When `the retired webhook secret` is not set, the route accepts **any** POST and acts on
...
- **Why it matters:** Without the secret, an attacker who can reach the endpoint can forge approval/
  ...
  effects.
- **Suggested fix:** Make the secret mandatory — if `the retired webhook secret` is unset, reject with
...
- **Severity:** MEDIUM
- **File:** `src/app/api/the old request app/webhook/route.ts:57-65`
...
- **Severity:** LOW
- **File:** `src/app/api/the old media server/image/[itemId]/route.ts:58` (and `next: { revalidate: 3600 }` at
  `:48`)
- **What's wrong:** Responses are returned with `Cache-Control: public, max-age=3600, immutable`.
  ...
  is replaced in the old media server. `immutable` tells browsers never to revalidate within the TTL, so swapped
  posters can be stale for up to an hour and the `public` directive permits shared/CDN caches to
  store the old media server artwork. Minor correctness/freshness issue; also note artwork is cached publicly
...
- **Suggested fix:** Drop `immutable` (keep `max-age`), or include the the old media server image tag/hash in
  the cache key so a changed image gets a new URL. Consider `private` once the route is auth-gated.

### A13-10 — Proxies buffer entire upstream response into memory (no streaming) for external automation
- **Severity:** LOW
- **File:** `the movie automation suite/[...path]/route.ts:34-35`, `the TV automation suite/[...path]/route.ts:36-37`,
  `the subtitle automation suite/[...path]/route.ts:35-36`
- **What's wrong:** The proxy fully reads the upstream body (`res.json()` / `res.text()`) and
  re-serializes it via `NextResponse.json(...)`. For large external automation payloads (e.g. full `/movie` or
  ...
  the pattern is known — just not applied to the external automation proxies.
...
- **Severity:** LOW
- **File:** all upstream `fetch` callers — `lib/{the movie automation suite,the TV automation suite,the subtitle automation suite,the old media server}/client.ts`, the four
  `[...path]` proxies, `the old media server/image` route
- **What's wrong:** Every call uses the global `fetch` with no shared keep-alive agent/dispatcher.
  ...
  external automation GET (`lib/the movie automation suite/client.ts:25`, etc.) means no response reuse either. [unverified] exact
  pooling behavior depends on the undici version bundled with Next 16.
- **Why it matters:** Minor throughput/latency overhead on bursty dashboards that fan out many external automation
...
- **Severity:** LOW
- **File:** `the movie automation suite/[...path]/route.ts:18-25`, `the TV automation suite/[...path]/route.ts:18-27`,
  `the subtitle automation suite/[...path]/route.ts:19-26`
...
- **API keys are server-only and never returned.** Keys come from `process.env` in each `client.ts`
  ...
  server→the old media server leg only). No success-path response includes a key. (Error bodies — A13-04 — leak
  internal URLs but not the keys.)
- **the old request app webhook HMAC is computed over the raw body before JSON parse** (`route.ts:75,96`), which is
  ...
  SQL injection.
- **the old media server path-segment params** (`seasonId`, series `id` in the seasons route) are passed through
