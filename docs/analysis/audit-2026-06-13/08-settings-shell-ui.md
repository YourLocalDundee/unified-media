...
page except theme (10 controls), 9 of 11 Playback prefs, the Advanced "the old media server URL override",
...
### A8-M2 — Advanced "the old media server URL Override" is written but never read
**Severity:** MEDIUM
**File:** `src/app/settings/advanced/page.tsx:16-31`

**What's wrong:** `savethe old media serverUrl` writes/removes `unified-the old media server-url-override` in
...
page's own comment claims it "lets a user point the browser at a different the old media server base URL
...
`/api/the old media server/...`, so a client-only override may be architecturally moot.
...
- **Media settings page** (`MediaSettingsClient.tsx`) — admin-gated, parallel
  ...
  calls to the the indexer bridge proxy.
