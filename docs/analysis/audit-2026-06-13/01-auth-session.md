...
**What's wrong:** `return ALLOWED_ORIGINS.some(o => origin === o || origin.startsWith(o))`. With `NEXT_PUBLIC_APP_URL=https://<old-app-host>`, an attacker-controlled origin `https://<old-app-host>.evil.com` passes `origin.startsWith('https://<old-app-host>')`. So a malicious page on that host satisfies the only CSRF check the app has. The `startsWith` branch appears intended to allow ports, but it admits arbitrary suffixes.
