...
5. **Blind body/param relays** to the old media server / the download client with no shape validation
   (`the old media server/sessions/*`, `requests/[id]/grab` magnet override).
...
- `src/app/api/the old media server/sessions/progress/route.ts:10`, `…/playing/route.ts`, `…/stopped/route.ts`
...
### A19-M6 — `the old media server/sessions/*` blindly relay the request body to the old media server (no shape/type validation)
**Severity:** MEDIUM
**Files:** `src/app/api/the old media server/sessions/progress/route.ts:10-11`, `…/playing/route.ts`, `…/stopped/route.ts`
**What's wrong:** `const body = await req.json(); await the old media serverFetch('/Sessions/Playing/Progress',{body:JSON.stringify(body)})`. No try/catch, no field validation — the entire client object is forwarded to the old media server's session API. (Report 14 flagged the *missing auth*; this finding is the *unvalidated relay + unhandled-JSON 500*.)
**Why it matters:** Malformed JSON → 500; otherwise an arbitrary attacker-shaped object reaches the old media server's `/Sessions/*` endpoints. No allowlist of fields (ItemId, PositionTicks, etc.).
**Suggested fix:** Parse defensively, validate the handful of fields the old media server needs, and forward only those.

### A19-M7 — `the old media server/subtitles/[itemId]/[streamIndex]`: params interpolated into upstream URL with no validation
**Severity:** MEDIUM
**File:** `src/app/api/the old media server/subtitles/[itemId]/[streamIndex]/route.ts:14-16`
**What's wrong:** `itemId` and `streamIndex` are placed straight into `${LEGACY_MEDIA_URL}/Videos/${itemId}/Subtitles/${streamIndex}/Stream.vtt` with no integer/format check (contrast the sibling `media/subtitles/embedded/[id]/[streamIndex]` which does `Number.isInteger(...) && >=0`). Path segments are not URL-encoded.
**Why it matters:** A crafted `streamIndex` like `0/../../Users` (URL-encoded) could alter the upstream path; at minimum non-integer values produce malformed upstream requests. Input-shape gap distinct from the auth gap report 13 noted for the the old media server metadata relays.
...
### A19-L3 — `the old media server/image/[itemId]`: `type`/`index`/`width` unvalidated in upstream URL
**Severity:** LOW
**File:** `src/app/api/the old media server/image/[itemId]/route.ts:15-34`
...
**Why it matters:** Mostly cosmetic (bad artwork / 404 from the old media server); minor SSRF surface is bounded by the fixed `LEGACY_MEDIA_URL` host.
