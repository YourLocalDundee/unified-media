...
(the the old request app webhook) is reachable unauthenticated when the optional secret is unset. No prototype
...
- Severity: Low
- File: `src/lib/media-server/scanner.ts:114` (`itemTitle`), `:116`; `src/lib/automation/grabber.ts:298` (`item.title`); `src/lib/indexer/index.ts:58` (`indexerName`), `:177-188`; `src/app/api/the old request app/webhook/route.ts:104-108,134,188` (`notification_type`, `subject`, `requestedBy_username`)
- What's wrong: these log statements interpolate untrusted strings (parsed filenames, release titles, indexer names, and the webhook's attacker-supplied `subject`/`username`/`notification_type`) directly into `console.*` / `process.stderr.write` without stripping CR/LF. The the old request app webhook (`webhook/route.ts`) is explicitly unauthenticated (`route.ts:5`) and only logs a warning instead of rejecting when `the retired webhook secret` is unset (`:89-91`), so an external caller can inject `\n`-laden values into the logs without credentials in that configuration.
