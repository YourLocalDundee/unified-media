...
- What's wrong: The scan dedupe query keys on `(legacy_item_id, language, status != 'failed')` only,
...
- Why it matters: Corrupt/garbage subtitles get auto-loaded by the old media server/player naming convention; a
...
- What's wrong: `next.config.ts` carefully whitelists `image.tmdb.org` and the the old media server host in
