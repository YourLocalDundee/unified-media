...
`getPlaybackData()` in `src/lib/the old media server/playback.ts`:
...
3. Builds `availableQualities: QualityOption[]`:
   ...
   - Each lower-quality URL is built by setting `MaxWidth`, `MaxHeight`, `VideoBitrate` on
     `hlsTranscodeUrl`.
4. Always computes `hlsTranscodeUrl` — if the old media server returns a `TranscodingUrl`, uses it; otherwise
