...
|---|---|---|---|---|
| 1 | Indexer Aggregation (the indexer bridge replacement) | ✅ Complete | 25–35h | p-limit replaced with manual semaphore |
| 2 | Download Automation (the TV automation suite + the movie automation suite replacement) | ✅ Complete | 60–80h | MVP: movies + TV, 3 default quality profiles |
| 3 | Request Bridge (the old request app → external automation link replacement) | ✅ Complete | 8–12h | Webhook receiver + availability poller |
| 4 | Subtitle Management (the subtitle automation suite replacement) | ✅ Complete | 30–40h | Daily scan + download cron; SUBTITLE_MEDIA_ROOT required for disk writes |
| 5 | Media Server (the old media server replacement) | ✅ Complete | 120–200h | Scanner, TMDB enricher, HLS transcode, playback sessions, watch state |
...
| 7 | Native Request Management (the old request app requests replacement) | ✅ Complete | 12–18h | media_requests table; search via TMDB; RequestButton; admin approve/decline |

## the old media server Migration — Complete (2026-05-30)

Zero unconditional the old media server API calls remain from pages or components. The migration is 100% done.

### What was completed

**VideoPlayer.tsx** — All the old media server fallback fetch calls removed. Progress reporting (`progressApiUrl`), next-episode lookup (`nextEpisodeApiBase`), and subtitle serving (`subtitleApiBase`) are native-only props with no the old media server defaults. Component warns and skips gracefully if props are not set.

**Admin server-status** — the old media server health check removed. Now uses native DB reachability (`SELECT 1`) and `MEDIA_ROOT` filesystem accessibility (`fs.access`).
...
**Dead code removed** — `the old media serverSeasonList.tsx` deleted. `the old media serverPerson` and `the old media serverStudio` types removed from `the old media server/types.ts`.

**`getItemsByType()`** — Now accepts an optional `year?: number` parameter for year filtering.

**EpisodeCard, EpisodeCarousel, SeriesSection** — All moved off the old media server types and routes to native `NativeEpisode`/`NativeSeason` types and `/api/media/*` routes.

**subtitle/scanner.ts** — No longer uses `the old media serverFetch`; queries SQLite directly.

**continue-watching/route.ts** — Native SQL, no the old media server imports.

**`the old media serverError` class** — Added to `src/lib/the old media server/client.ts`.
...
src/lib/automation/
  ...
  availability.ts   — checkAvailability, searchthe old media server

src/lib/the old request app/api.ts          — added getRequest(id)
src/app/api/the old request app/
  request/[id]/approve/route.ts  — now calls onRequestApproved after proxying to the old request app
                                    ALSO added requireAdmin() (was unguarded — security fix)
  webhook/route.ts               — the old request app webhook receiver
...
### Bridge flow
```
User approves in the old request app UI
  → POST /api/v1/request/{id}/approve (the old request app internal)
  → OR admin approves via unified frontend: POST /api/the old request app/request/{id}/approve
       → calls onRequestApproved(the old request appRequest)
       → INSERT INTO monitored_items (idempotent by tmdb_id + type)
  → OR the old request app webhook fires POST /api/the old request app/webhook
       → MEDIA_APPROVED / REQUEST_APPROVED → onRequestApproved

Phase 2 cron (*/15) picks up the wanted item → grab dispatched
Phase 2 cron (*/30) finds item in the old media server → status = 'imported'
```

### New env vars
| Variable | Required | Where used |
|---|---|---|
| `LEGACY_MEDIA_USER_ID` | Yes (poller) | `availability.ts` — the old media server `/Users/{id}/Items` search |
| `the retired webhook secret` | Optional | `webhook/route.ts` — X-Webhook-Signature verification |
...
src/lib/subtitle/
  ...
  scanner.ts         — scanLibrary() queries SQLite directly (no longer uses the old media serverFetch)
...
CREATE TABLE subtitle_wants (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  legacy_item_id TEXT NOT NULL, legacy_item_type TEXT NOT NULL,
  ...
  UNIQUE (legacy_item_id, language, forced, hi)
)
```
...
a retired env var=<from the old request app settings>

# New for Phase 3 (optional)
the retired webhook secret=<random string>         ← configure same value in the old request app webhook settings
