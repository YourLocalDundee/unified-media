...
- **Web Push for "request available" (finish the loop).** The the old request app webhook (`/api/the old request app/webhook`)
...
- **Calendar / upcoming view.** the external automation services integration already exists for monitoring status; a
  unified "coming soon / recently aired" calendar (episodes airing this week, movies releasing) is a
  read-only aggregation over data the external automation layer already exposes.
...
## From the automation gap analysis (the TV automation suite / the movie automation suite / the indexer bridge mining)

These came out of the source purge; ranked detail is in retired design notes.
...
- ~~**Standard category mapping + manual search.**~~ **SHIPPED 2026-07-11** — since every indexer here
  is a Torznab endpoint (Torznab), standard category IDs already work correctly against
  ...
  search` is the manual-search debug tool from retired design notes: query + category
...
- **Wire up FlareSolverr.** Discovered 2026-07-11 while auditing retired design notes:
  ...
  the indexer bridge-bridge row (`discovery.ts`) hardcodes it `0` — the indexer bridge handles Cloudflare-gated trackers
  with its own FlareSolverr integration before we ever see the Torznab response, so bridge rows never
  need ours. The gap only bites a *direct* (non-the indexer bridge) Torznab indexer added later that's
...
## Party Play polish (from watchparty / a reference project mining)
...
- **Tri-state synced/syncing/waiting badge.** The `position_ticks` median and readiness state are
  ...
  is pure UI. See retired design notes for the vocabulary.
