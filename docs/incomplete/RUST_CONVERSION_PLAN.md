...
5.4 Run both frontends in parallel on separate hostnames during the port (<legacy-app-host> stays Next.js, <rs-host> is Leptos) so nothing is broken while pages migrate. Flip DNS/routing when parity is reached.
