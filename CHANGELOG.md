...
- **Standard category widening + manual search (category management, part 2).** New
  ...
  New `/admin/indexers/search` — a manual-search debug page (retired design notes): query +
...
- **Indexer capabilities probe (category management, part 1).** `testIndexer()` now parses the
  ...
  category management UI" (retired design notes); mapping to the Newznab standard
...
- **`indexers.name` had no unique constraint, despite every insert path assuming one.**
  ...
  2026-07-11 when re-running the the indexer bridge bridge to pick up newly-added trackers silently
...
- **the indexer bridge-direct admin surface removed (2026-07-10) — first piece of the the indexer bridge replacement.**
  Deleted `/settings/media`, `/api/the indexer bridge/[...path]`, and `lib/the indexer bridge/` (`client.ts`, `types.ts`,
  and the already-dead `api.ts`). That page duplicated `/admin/indexers` (the native `indexers` table)
  with a worse UI and no health/rate-limit persistence — toggling a the indexer bridge indexer there never
  affected what our own search fan-out actually queries. `/admin/indexers` is now the only indexer
  admin surface. `a retired env var`/`a retired env var` are still read once at first boot by
  `src/lib/indexer/discovery.ts` to seed native `indexers` rows from the indexer bridge's per-tracker Torznab
  endpoints (the pragmatic "keep the indexer bridge as a Torznab source" call from
  retired design notes) — that one-shot bridge is not a live proxy. Full the indexer bridge
  independence (porting the the YAML indexer engine engine) stays deferred as high-effort/low-value for a home
  server.
- **Purged upstream `sources/` reference material (147 MB, 7 directories).** The native stack is
  ...
  mined before deletion: retired design notes is the permanent record covering all
...
- **unified-media is now fully Authentik-free in docs too.** Authentik left the request path at
  ...
  release notes and the pre-build the old request app-analysis suggestion. No code changed (there was none to
...
- **Renamed `subtitle_wants.legacy_item_id` / `legacy_item_type` → `media_item_id` /
...
- Scrubbed the old media server from CLAUDE.md (§1 overview, §2 architecture note, §3 service table, §4 tech
  stack, §7 gotchas, §8 env), SETUP.md, and in-code comments/UI strings. Marked
  retired design notes as archived. Moved the optional "the old media server user linking"
...
- **Quick + interactive grab**: Interactive torrent picks on 48hr requests now grab immediately without admin approval, matching the movie automation suite's interactive search behavior.
...
### Added
- **the old request app webhook receiver** (`src/app/api/the old request app/webhook/route.ts`) — implements the Phase 3 bridge endpoint. Handles `MEDIA_APPROVED` / `REQUEST_APPROVED` (idempotency via `findItemForRequest`, creates `monitored_item`, fires immediate grab as fire-and-forget), `MEDIA_AVAILABLE` (updates `media_requests.status`), and all other event types (200 ignored). HMAC-SHA256 signature verification via `timingSafeEqual` when `the retired webhook secret` is set.
...
- **`SUBTITLE_MEDIA_ROOT=/media`** added to `.env.local` — required for the subtitle downloader to write `.srt` files alongside media files using the the old media server/Plex naming convention.
...
- **CLAUDE.md** — corrected Next.js version (`14+` → `16+`), node base image (`22-slim` → `24-slim`), proxy file convention (middleware.ts → proxy.ts), `QBT_URL` → `QBIT_URL` in compose example, admin seeding behavior, `external automation` network mode, the old media server image proxy path, 21 DB tables listed, admin nav extended with Quality Profiles + Settings, BunkerWeb WAF table completed with all 5 disabled modules.
...
### Added
- **Independence build Phase 5 — Native media server** (`src/lib/media-server/`) — TypeScript media server replacing the old media server as the primary browse and playback backend. File scanner (`scanner.ts`), TMDB enricher (`enricher.ts`), ffprobe-based probe (`probe.ts`), HLS transcoder (`transcode.ts`), library query layer (`library.ts`), playback data builder (`playback.ts`), TMDB client (`tmdb.ts`). Background scanner starts from `src/instrumentation.ts`. Stores items in `media_items` and `media_watch_state` SQLite tables.
- **Independence build Phase 6 — Browse and watch wired to native media server** — `/browse` and `/browse/[id]` now query the native media server library instead of the old media server. `/api/media/items/[id]`, `/api/media/image`, `/api/media/filters`, `/api/media/items/[id]/similar` routes added. Stream proxy at `/api/the old media server/stream/[...path]` retained as pass-through for the old media server-backed items during migration.
- **Independence build Phase 7 — Native request management** (`src/lib/requests/`) — request creation, approval, and status tracking backed by `media_requests` SQLite table. Replaces the old request app as the request backend. Admin requests page at `/admin/requests` with approval and decline actions.
...
### Changed
- `continue-watching/route.ts` — rewritten to query `media_items` + `media_watch_state` via SQL; the old media server imports removed.
- `EpisodeCard.tsx`, `EpisodeCarousel.tsx`, `SeriesSection.tsx` — switched from the old media server SDK types to native `NativeEpisode` / `NativeSeason` types; images served via `/api/media/image?path=`.
- `VideoPlayer.tsx` progress reporting — `reportStart`, `reportProgress`, `reportStop` fall back gracefully when `progressApiUrl` is not set (logs warning, returns); no longer falls back to the old media server session routes.
...
- **Independence build Phase 2 — Download automation** (`src/lib/automation/`) — want-list-driven grab loop replacing the external automation services. `monitored_items` and `grab_history` tables. `grab_results` table stores search candidates per grab attempt. Quality profiles system: `quality_tiers` (19 canonical tiers seeded at startup), `custom_formats`, `quality_profile_formats`. Automated grabber (`grabber.ts`), scheduler (`scheduler.ts`), parser (`parser.ts`), quality scorer (`quality.ts`). Admin UI at `/admin/automation` with monitored items list, grab history, manual Grab Now action, Add Item modal.
- **Independence build Phase 3 — Request bridge** (`src/lib/automation/bridge.ts`) — links the request system to the automation grab loop. the old request app webhook receiver at `/api/the old request app/webhook` for `Request Approved` + `Media Available` events. Admin bridge status page at `/admin/automation/bridge`.
- **Independence build Phase 4 — Subtitle management** (`src/lib/subtitle/`) — OpenSubtitles v3 integration replacing the subtitle automation suite. `subtitle_wants` table tracks wanted/downloaded/skipped/failed subtitle items. Library scanner discovers media without subtitles. Download job fetches `.srt` files and writes them to `SUBTITLE_MEDIA_ROOT`. Admin UI at `/admin/subtitles` with scan, download, and status management.
...
- **`subtitle_wants` DB table** — tracks subtitle download state: `legacy_item_id`, language, forced/HI flags, `status`, `subtitle_path`.
- **`quality_tiers`, `custom_formats`, `quality_profile_formats` DB tables** — seeded with 19 canonical tiers at startup.
- **New env vars** — `OPENSUBTITLES_API_KEY` (required for subtitle phase), `SUBTITLE_LANGUAGES` (optional, default `en`), `SUBTITLE_MEDIA_ROOT` (optional, required for disk writes), `the retired webhook secret` (optional).
...
- **Media server health in server-status** — `GET /api/admin/server-status` gains a `media: { ok, root }` field that verifies `MEDIA_ROOT` is accessible. the old request app and qBit checks retained.
...
- **the old media serverError class** — added to `src/lib/the old media server/client.ts` with `status`, `isAuthError`, `isNotFound`, `isServerError` getters.
- **Config validation at startup** — `instrumentation.ts` gains `validateConfig()`: fatal on missing `ADMIN_USERNAME`/`ADMIN_PASSWORD`, warns on missing the old media server vars.
...
### Changed
- **`continue-watching/route.ts`** — Rewrote to use native SQL joining `media_items` and `media_watch_state`. All the old media server imports removed.
- **`EpisodeCard.tsx`** — Replaced `the old media serverEpisode` type with `NativeEpisode` (snake_case fields). Images now served via `/api/media/image?path=...`.
- **`EpisodeCarousel.tsx`** — Calls `/api/media/seasons/${seasonId}/episodes` instead of the the old media server route. Uses `NativeEpisode`.
- **`SeriesSection.tsx`** — Replaced `the old media serverSeasonShape` with `NativeSeason { id, title, season_number }`.
...
- **`subtitle/scanner.ts`** — Removed `the old media serverFetch` import; now queries `media_items` via SQLite directly.
...
- **Auth header format** — Fixed in 3 the old media server proxy routes.
- **`VideoPlayer.tsx` progress reporting** — `reportStart`, `reportProgress`, and `reportStop` no longer have `else` branches falling back to `/api/the old media server/sessions/*`. If `progressApiUrl` is not set they log a warning and return. Removed unused `mediaSourceId`, `playSessionId`, and `isHls` from callback dep arrays.
- **`VideoPlayer.tsx` fallbacks** — `nextEpisodeApiBase` and `subtitleApiBase` no longer fall back to the old media server routes. Both log a warning and skip if not set.

### Removed
- **`src/components/media/the old media serverSeasonList.tsx`** — Deleted. Orphaned component; not imported by any page.
- **the old media server health check** — `LEGACY_MEDIA_URL/System/Info/Public` check removed from `GET /api/admin/server-status`. Replaced by the native media root accessibility check.
- **the old media server env var references** — Zero `LEGACY_MEDIA_*` env var references remain outside `src/lib/the old media server/` and `src/app/api/the old media server/`.
...
### Added
- **external automation TypeScript clients** — `src/lib/the TV automation suite/`, `src/lib/the movie automation suite/`, `src/lib/the indexer bridge/`, `src/lib/the subtitle automation suite/` each with `client.ts` (fetch wrapper with API key injection), `types.ts` (full typed interfaces from live API), and `api.ts` (typed helper functions)
- **API proxy routes** — `src/app/api/{the TV automation suite,the movie automation suite,the indexer bridge,the subtitle automation suite}/[...path]/route.ts` transparent authenticated proxies; all four gated by `requireAuth()` so API keys never reach the browser
- **Media settings page** — `/settings/media` (admin-only via `requireAdmin()`) with 4 tabs: Indexers (the indexer bridge — enable/disable toggle, test button, count badge), TV (the TV automation suite quality profiles + root folders), Movies (the movie automation suite quality profiles + root folders), Subtitles (the subtitle automation suite providers + version info); all tabs gracefully degrade if a service is down
- **Admin requests page** — `/admin/requests` with approval modal that loads quality profiles from the external automation services at open time; approve and decline actions update row state inline without page reload; filter tabs (Pending/All/Approved/Declined) load server-side enriched data
- **Decline API route** — `POST /api/the old request app/request/[id]/decline`
...
- **env vars** — `LEGACY_TV_URL`, `LEGACY_TV_API_KEY`, `LEGACY_MOVIE_URL`, `LEGACY_MOVIE_API_KEY`, `a retired env var`, `a retired env var`, `LEGACY_SUBS_URL`, `LEGACY_SUBS_API_KEY`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM` added.

### Changed
- Settings nav — added "Media" tab between Torrent and Advanced
- Admin nav — added "Requests" tab between Invites and Watch Activity
- the old media server image proxy — removed `force-dynamic` (was overriding `next: { revalidate: 3600 }`); browser now correctly caches images for 1 hour
...
- **About page** — removed the old media server/the old request app/qBittorrent version rows; removed Service Links section; replaced static blurb with parsed CHANGELOG accordion
...
- **the old media server image proxy** — accepts `?type=Backdrop&index=0` and constructs `/Images/Backdrop/{index}` path correctly
...
- `PlaybackData.nativeWidth` / `nativeHeight` — extracted from the old media server MediaStream video track
...
- `getPlaybackData` fetches `Chapters` from the old media server item metadata and includes in `PlaybackData.chapters`
...
- **MediaChapters** — chapter list from the old media server with current-chapter highlight and prev/next navigation; VLC analogue: `chapterNext()`, `chapterPrev()`, chapters TrackListModel
...
- Stream proxy `/api/the old media server/stream/[...path]` requires valid session; returns 401 without one
...
- `src/app/api/the old media server/stream/[...path]/route.ts` — authenticated HLS/stream proxy with manifest URL rewriting
...
- **Browse detail page** — `/browse/[id]` with full the old media server metadata, watch/request actions, episode list for series.
- **Settings layout** — Tabbed settings shell at `/settings` covering Playback and (stub) Account pages.
- **`getPlaybackData`** — Moved to `src/lib/the old media server/playback.ts` with direct-play and HLS transcoding URL resolution.

### Changed
- the old media server playback API route now imports from `lib/the old media server/playback` (fixed build error: route files cannot export non-HTTP named exports).
...
### Added
- **Full service integrations** — the old media server library browsing, the old request app request management, qBittorrent download queue.
- **Search** — Unified search across the old media server library and the old request app discover at `/search`.
...
- **Image proxy** — `/api/the old media server/image/[id]` route injects auth header so images render without embedding API keys in HTML.
...
- Environment variable wiring for the old media server, the old request app, qBittorrent.
