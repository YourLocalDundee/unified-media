// Indexer head-to-head comparison. Runs a
...
// Two rows for the same tracker rarely share an exact name (e.g. "Nyaa.si" vs "Nyaa") —
// case-insensitive contains-either-way covers the mismatches seen in practice while still
// matching exactly for the rest.
function suggestTwin(native: Indexer, all: Indexer[]): Indexer | null {
  const nativeName = native.name.toLowerCase()
  for (const c of all) {
    if (c.id === native.id) continue
    const other = c.name.toLowerCase()
    if (other.includes(nativeName) || nativeName.includes(other)) return c
  }
  return null
}
...
export default function CompareIndexersPage() {
  ...
  function handlePickA(value: string) {
    ...
    if (!native) return
    const suggestion = suggestTwin(native, indexers)
    if (suggestion) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-foreground">Compare Indexers</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            {idA !== '' && idB === '' && !loadingIndexers && (
              <p className="mt-1 text-xs text-muted-foreground">No likely twin found for this tracker — pick a second indexer manually to compare.</p>
            )}
          </div>
        </div>
