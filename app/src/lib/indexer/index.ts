...
export async function parseXml(xml: string, indexerName: string): Promise<TorznabResult[]> {
  ...
  for (const raw of items) {
    ...
    // sometimes a bare hash-bearing URL, sometimes a full magnet URI itself — some Torznab feeds
    // put the whole magnet: URI in <guid>). normalizeInfoHash both lowercases and
    ...
    // 2026-07-13: one feed emits uppercase hex (case alone broke matching); another's
    // guid carries a Base32 magnet that the old hex-only guid regex missed entirely.
...
export async function fetchTorznabResults(
  ...
  // page size — found live 2026-07-13 comparing two rows for the same tracker: identical query,
  // but one side silently got 50 results to the other's 100, because only one request sent an
  // explicit limit. 100 matches the cap every native JSON-API adapter in this app already uses.
