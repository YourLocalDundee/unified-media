...
describe('compareIndexers', () => {
  ...
  it('dispatches a registered adapter type directly instead of hitting the torznab_url', async () => {
    ...
    const torznabIndexer = mockIndexer({ id: 4, name: 'YTS (Torznab)', torznab_url: 'https://indexer.example.com/torznab' })
    ...
    const result = await compareIndexers(nativeIndexer, torznabIndexer, { q: 'movie' })
