...
export function RequestButton({
  ...
  if (currentStatus && currentStatus !== 'declined') {
    if (uiState === 'success' && currentStatus === 'pending') {
      ...
      return (
        <span className="inline-flex items-center rounded-lg px-3 py-2 text-sm font-medium bg-green-900/60 text-green-300">
          Requested!
        </span>
      )
    }
  ...
  if (currentStatus === 'declined') {
    return (
      <button
        ...
        className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium bg-red-900/40 text-red-400 hover:bg-red-900/70 hover:text-red-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {uiState === 'loading' && (
  ...
  if (uiState === 'error') {
    return (
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={submitRequest}
          className="inline-flex items-center rounded-lg px-3 py-2 text-sm font-medium bg-zinc-800 text-white hover:bg-zinc-700 transition-colors"
        >
          Retry
        </button>
      </div>
  ...
  return (
    <button
      onClick={submitRequest}
      disabled={uiState === 'loading'}
      className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium bg-zinc-800 text-white hover:bg-zinc-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {uiState === 'loading' && (
        <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
